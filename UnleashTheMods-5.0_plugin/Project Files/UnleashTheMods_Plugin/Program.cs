...
class Program
{
    static void Main(string[] args)
    {
        ...
        string modsInputDirectory = args.Length > 1 ? args[1] : Path.Combine(baseDirectory, "mods");
        string stagingDirectory = args.Length > 2 ? args[2] : Path.Combine(baseDirectory, "staging_area");
        string outputDirectory = args.Length > 3 ? args[3] : modsInputDirectory;

        Console.WriteLine($"[LOG] Source directory: {sourceDirectory}");
        Console.WriteLine($"[LOG] Mods input directory: {modsInputDirectory}");
        Console.WriteLine($"[LOG] Staging directory: {stagingDirectory}");
        Console.WriteLine($"[LOG] Output directory: {outputDirectory}");

        if (!Directory.Exists(sourceDirectory) || !Directory.Exists(modsInputDirectory))
        {
        ...
        var moddedFiles = modCacher.LoadAndProcessMods(modsInputDirectory);
        ...
        Packager.PackageFiles(sourceDirectory, stagingDirectory, outputDirectory, finalFileContents, mergeSummary); // CZT
