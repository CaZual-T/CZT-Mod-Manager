...
{
    public static void PackageFiles(string sourceDirectory, string stagingDirectory, string profileModsDirectory, Dictionary<string, byte[]> finalFileContents, Dictionary<string, List<string>> mergeSummary)
    {
        Console.WriteLine("\n\n--- Merge Completed ---");

        Console.WriteLine("\n--- Creating Final .pak File ---");

        // CZT PATCH - Use a clean temp staging directory each run
        ...
        {
            // Normalize path separators to OS standard before combining paths
            string normalizedKey = fileEntry.Key.Replace('/', Path.DirectorySeparatorChar)
                                                 .Replace('\\', Path.DirectorySeparatorChar);
            string fullPath = Path.Combine(stagingDirectory, normalizedKey);
        ...
        string finalPakPath = Path.Combine(profileModsDirectory, "data7.pak");
        ...
        // Clean up staging directory after pak is built
        try { Directory.Delete(stagingDirectory, true); }
        catch { /* ignore cleanup errors */ }

        Console.ForegroundColor = ConsoleColor.Green;
        Console.WriteLine($"\nSUCCESS! All mods have been merged and saved as '{Path.GetFileName(finalPakPath)}' in the profile mods folder: {profileModsDirectory}");
        Console.ResetColor();
        ...
        if (mergeSummary.Any())
        {
            ...
            var groupedByDirectory = mergeSummary
                .Select(kvp => new { Directory = Path.GetDirectoryName(kvp.Key)?.Replace('/', '\\'), FileName = Path.GetFileName(kvp.Key), Mods = kvp.Value })
        ...
        // CZT Patch - Clean staging directory leftovers if it still exists
        if (!Directory.Exists(stagingDirectory))
            return;
