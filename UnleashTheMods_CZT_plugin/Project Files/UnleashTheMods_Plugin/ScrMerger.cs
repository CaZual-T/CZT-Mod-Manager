...
public class ParameterContent : INodeContent
{
    ...
    {
        public required ScrMerger.ScriptNode Node { get; set; }
        public int OriginalOrder { get; set; }
...
private ScriptNode RecursiveMerge(ScriptNode originalNode, List<(string Source, ScriptNode Tree)> modNodes, MergeReporter reporter)
{
    ...
    foreach (var signature in allSignaturesOrdered)
    {
        ...
        if (originalContent == null)
        {
            ...
            foreach (var newContent in distinctNewContents)
            {
                if (newContent.Content is ChildNodeContent newChild)
                {
                    ...
                    newChild.Node.HeaderComments.Insert(0, $"// -- [UTM-CZT Merge] Block added from {newContent.Source} --");
                }
        ...
        {
            var actualChanges = modContents.Where(mc => mc.Content is ParameterContent p && p.Value.Trim() != originalParam.Value.Trim()).Select(mc => (Param: (ParameterContent)mc.Content!, Source: mc.Source)).ToList();
            ...
            {
                var distinctChanges = actualChanges.GroupBy(c => c.Param.Value.Trim()).Select(g => (Param: g.First().Param, Sources: g.Select(c => c.Source).ToList())).ToList();
                ...
                var paramToAdd = new ParameterContent { Key = chosenParam.Key, OriginalOrder = chosenParam.OriginalOrder, Value = AppendUtmComment(chosenParam.Value, $"[UTM-CZT Merge] updated from {sourceForComment} (OG Value: {ogValue})") };
                mergedNode.Contents.Add(paramToAdd);
            }
        }
        else if (originalContent is ChildNodeContent originalChild)
        {
            var modChildren = modContents
                .Where(m => m.Content is ChildNodeContent)
                .Select(m => (m.Source, Tree: ((ChildNodeContent)m.Content!).Node))
                .ToList();
            if (modChildren.Any())
            {
                ...
                if (signatureChange != default)
                {
                    ...
                    mergedChildNode.HeaderComments.Add($"// -- [UTM-CZT Merge] Block from {signatureChange.Source} --");
                }
