...
def run(czt_root_folder=None, profile_name=None, mods_dir=None, config_path=None, app_version=None):
    try:
        ...
        global_configs = importlib.import_module("var_global.Global_Configs")
        GAME_PROFILES = getattr(global_configs, "GAME_PROFILES", None)
        if GAME_PROFILES is None:
            print("[PLUGIN] Could not access GAME_PROFILES in Global_Configs.")
            return
        profiles_to_add = {
        ...
        for profile_name, profile_data in profiles_to_add.items():
            if profile_name in GAME_PROFILES:
                print(f"[PLUGIN] Profile '{profile_name}' already exists in memory.")
            else:
                GAME_PROFILES[profile_name] = profile_data
                print(f"[PLUGIN] Profile '{profile_name}' has been added as a usable profile.")
        print(" > Custom profiles will remain in-memory until plugin is removed.")
    except Exception as e:
        print(f"[PLUGIN] Error adding profile to GAME_PROFILES: {e}")
