                    ...
                    if fname.endswith(".item"):
                        ...
                        try:
                            ...
                            if install_dir and exe_name and exe_name.strip() and game_name and cfg and save_cfg and czt_root_folder:
                                ...
                                final_profile_path = build_install_dir_path(plugin_api, install_dir, profile_subfolder, game_name=current_profile)
...
def build_install_dir_path(plugin_api, install_dir, profile_subfolder, game_name=None):
    ...
    # If install_dir already contains the game name (or a variant), avoid re-adding the game folder
    elif subfolder_parts and game_name and normalize_name(game_name) in normalize_name(install_dir_last):
        subfolder_to_append = os.path.join(*subfolder_parts[1:]) if len(subfolder_parts) > 1 else ''
    else:
