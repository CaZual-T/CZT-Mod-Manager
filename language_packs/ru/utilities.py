...
MSG_HOTKEY_CONTROLS = (
    ...
    "    > F7           : Привязать моды (без запуска игры).\n"
    "    > F8           : Отвязать моды / безопасный режим (без запуска).\n"
...
LOG_HOTKEY_LINK_DONE = "[LINK] {game_name}: моды успешно привязаны к {path} (Hotkey - без запуска)"
LOG_HOTKEY_UNLINK_DONE = "[UNLINK] {game_name}: все моды успешно отвязаны."
...
LOG_LAUNCH_REFRESHING_SYMLINKS = "[LINK] Обновление симлинков..."
...
LOG_LOADER_RENAMED = "[Loader] {name} мода '{mod}' переименован -> {rel} для прокси-загрузки."
...
LOG_LOADER_NOT_FOUND = "[Loader] Встроенный загрузчик не найден в {path}; пропуск."
LOG_LOADER_INJECTED = "[Loader] Встроенный прокси-загрузчик '{name}' внедрён."
LOG_LOADER_INJECT_FAILED = "[Loader] Не удалось внедрить встроенный загрузчик: {error}"
...
LOG_FORZATECH_AUTOSCOPE_INDEXING = "[ForzaTech][autoscope] Индексация ванильного дерева media/ (первый запуск)..."
LOG_FORZATECH_AUTOSCOPE_INDEXED = "[ForzaTech][autoscope] Проиндексировано {files} ванильных имён файлов в {categories} категориях."
LOG_FORZATECH_AUTOSCOPE_UNNESTED = "[ForzaTech][autoscope] Уже преднастроенный мод '{mod}' вынесен из вложенной обёртки."
LOG_FORZATECH_AUTOSCOPE_MEDIA_TO_MEDIAPC = "[ForzaTech][autoscope] Верхняя папка 'media' нормализована в 'mediapc' для '{mod}'."
LOG_FORZATECH_AUTOSCOPE_LOADER_SKIPPED = "[ForzaTech][autoscope] '{mod}' содержит загрузчик .dll/.asi; autoscope пропущен."
LOG_FORZATECH_AUTOSCOPE_UNSCOPED = "[ForzaTech][autoscope] Не удалось определить категорию для {count} файл(ов) в '{mod}'; оставлены без области."
LOG_FORZATECH_AUTOSCOPE_RESCOPED = "[ForzaTech][autoscope] Переразмещено '{mod}': {count} файл(ов) -> mediapc/."
LOG_FORZATECH_AUTOSCOPE_UNRESOLVED = "[ForzaTech][autoscope] {count} файл(ов) в '{mod}' оставлены без области (неизвестная категория)."
LOG_FORZATECH_AUTOSCOPE_MOVE_FAILED = "[ForzaTech][autoscope] Не удалось переместить '{path}': {error}"
LOG_FORZATECH_AUTOSCOPE_REZIPPED_CAR = "[ForzaTech][autoscope] Бандл автомобиля переархивирован -> mediapc/Cars/{name}.zip"
LOG_FORZATECH_AUTOSCOPE_REZIP_FAILED = "[ForzaTech][autoscope] Ошибка переархивации бандла автомобиля '{name}': {error}"
LOG_FORZATECH_NO_ENABLED_MODS = "[ForzaTech] Нет включённых модов для развёртывания."
LOG_FORZATECH_REUSING_OVERLAY = "[ForzaTech] Подготовленные моды без изменений --> повторное использование существующего наложения."
LOG_FORZATECH_BUILT_OVERLAY = "[ForzaTech] Создано объединённое наложение: {count} файл(ов) из {mods} мод(ов)."
LOG_FORZATECH_DEPLOYED = "[ForzaTech] Развёрнуто {count} объект(ов) наложения в игре."
LOG_FORZATECH_JUNCTION_FAILED = "[ForzaTech] Не удалось создать соединение (junction) для {name}: {error}"
LOG_FORZATECH_JUNCTION_REFRESH_FAILED = "[ForzaTech] Не удалось обновить соединение (junction) для {name}: {error}"
LOG_FORZATECH_OVERLAY_LINK_FAILED = "[ForzaTech] Не удалось создать ссылку наложения для {path}: {error}"
LOG_FORZATECH_REMOVE_FAILED = "[ForzaTech] Не удалось удалить {path}: {error}"
LOG_FORZATECH_OVERLAY_CLEANUP_FAILED = "[ForzaTech] Не удалось очистить наложение для {path}: {error}"
LOG_FORZATECH_SWEPT_ORPHANS = "[ForzaTech] Удалено {count} мёртвых ссылок наложения, пропущенных манифестом."
LOG_FORZATECH_REMOVED = "[ForzaTech] Удалено {count} объект(ов) наложения из игре."
...
DLG_TITLE_SELECT_FILES_DOWNLOAD = "Выберите файл(ы) для скачивания"
