...
TIP_BROWSE_UNRAR = (
    "Укажите путь к UnRAR.exe (или WinRAR.exe), расположенному в любом месте на вашем ПК.\n"
    "Найдите файл или вставьте его полный путь в поле и нажмите Enter, чтобы сохранить."
)
TIP_NEXUS_API = (
...
MSG_UNRAR_PATH_INVALID_TITLE = "Неверный путь к UnRAR"
MSG_UNRAR_PATH_INVALID_TEXT = (
    "Этот путь не указывает на действительный UnRAR.exe или WinRAR.exe.\n"
    "Найдите файл или вставьте его полный путь и нажмите Enter."
)
...
# ============================================================
DLG_TITLE_CUSTOM_PROFILES = "Пользовательские профили"
LBL_CUSTOM_PROFILES_TITLE = "Пользовательские профили игр"
LBL_CUSTOM_PROFILES_SUBTITLE = (
    "Добавьте свои игры. Оставленные пустыми поля наследуют разумные значения по умолчанию движка."
)
LBL_EDITING = "Редактирование:"
LBL_PROFILE_NAME = "Имя профиля *"
LBL_ENGINE = "Движок *"
LBL_ENGINE_DESCRIPTION = "Описание:"
LBL_STEAM_APPID = "Steam App ID"
LBL_LAUNCH_EXE = "Исполняемый файл запуска"
LBL_GAME_PATH = "Путь к модам игры"
LBL_NEXUS_LINK = "Ссылка Nexus на игру *"
LBL_MOD_DIR_NAME = "Имя папки модов"
LBL_ROUTING = "Маршрутизация папок"
LBL_DEFAULT_MODS_DIR = "Размещать моды прямо в корневой папке профиля (без подпапки 'Mods')"
OPT_NEW_PROFILE = "＋ Новый профиль"
BTN_DELETE_PROFILE = "Удалить"
BTN_SAVE_PROFILE = "Сохранить профиль"
BTN_CLOSE = "Закрыть"

# Custom Profiles - input placeholders
PH_PROFILE_NAME = "напр.: Моя классная игра"
PH_STEAM_APPID = "Steam App ID (напр.: 1144200)"
PH_LAUNCH_EXE = "Game.exe"
PH_GAME_PATH = r"\Моя игра\Mods"
...
TIP_CP_PROFILE_SELECTOR = (
    ...
    "• '＋ Новый профиль' открывает пустую форму для создания новой игры.\n"
    "• При выборе существующего профиля загружаются его значения для редактирования или удаления.\n"
    "Здесь показаны только ВАШИ пользовательские профили --> встроенные профили нельзя редактировать\n"
    "или удалять из этого окна."
)
TIP_CP_PROFILE_NAME = (
    "Отображаемое имя вашей игры, показываемое в выпадающем списке профилей (обязательно).\n"
    "Оно также становится реальным именем папки в корне CZT, поэтому должно быть\n"
    "допустимым именем папки Windows.\n"
    "Разрешено: буквы, цифры, пробелы, дефисы, подчеркивания.\n"
    "Запрещено: < > : \" / \\ | ? * , имена, заканчивающиеся пробелом или точкой, и\n"
    "зарезервированные имена (CON, PRN, NUL, COM1…).\n"
    "Должно быть уникальным --> не может совпадать со встроенным или другим пользовательским профилем."
)
TIP_CP_ENGINE = (
    "Движок модов, который CZT использует для установки и связывания модов этой игры (обязательно).\n"
    "Выберите движок, соответствующий вашей игре (напр. Unreal Engine, Unity, Techland).\n"
    "Движок предоставляет умные значения по умолчанию (разрешенные типы файлов, способ связывания модов),\n"
    "поэтому вам редко нужно трогать расширенные поля ниже.\n"
    "Серый текст под этим полем кратко описывает выбранный движок."
)
TIP_CP_STEAM_APPID = (
    "Steam App ID игры (необязательно, только цифры).\n"
    "Найдите его в URL магазина Steam: store.steampowered.com/app/<APPID>/.\n"
    "CZT использует его для автоматического поиска игры через вашу библиотеку Steam.\n"
    "Оставьте пустым для игр не из Steam --> вы все равно можете задать пути вручную на\n"
    "вкладке Главного меню. Буквы или символы здесь будут отклонены при сохранении."
)
TIP_CP_LAUNCH_EXE = (
    "Исполняемый файл игры относительно папки игры (необязательно).\n"
    "Пример: Game.exe  или  Binaries\\Win64\\Game.exe.\n"
    "Разделяйте несколько вариантов запятыми --> CZT использует первый найденный.\n"
    "Используется кнопкой 'Запустить игру'; оставьте пустым, если запускаете игру сами."
)
TIP_CP_GAME_PATH = (
    "Фрагмент пути, помогающий обнаружить папку модов игры (Steam).\n"
    "Обычно это конец пути установки, напр. \\Моя игра\\Mods.\n"
    "Разделяйте несколько вариантов запятыми.\n"
    "Это подсказка для обнаружения, а не полный путь --> для полного ручного контроля задайте\n"
    "путь установки игры на вкладке Главного меню после сохранения."
)
TIP_CP_NEXUS_LINK = (
    "Страница Nexus Mods для этой игры (необязательно).\n"
    "Формат: https://www.nexusmods.com/<game>/mods/  где <game> - домен,\n"
    "показанный в URL сайта для этой игры.\n"
    "Включает встроенный браузер Nexus и ссылки 'открыть на Nexus' для этого профиля.\n"
    "Оставьте пустым, если игры нет на Nexus Mods."
)
TIP_CP_MOD_DIR_NAME = (
    "Имя папки на стороне игры, в которую связываются моды (необязательно).\n"
    "Обычные значения: Mods, Paks, source. По умолчанию 'Mods', если оставить пустым.\n"
    "Игнорируется, когда включено 'Размещать моды прямо в корневой папке профиля'.\n"
    "Если не уверены, оставьте значение по умолчанию --> движок обычно обрабатывает это правильно."
)
TIP_CP_ROUTING = (
    "Дополнительно: направляйте определенные папки верхнего уровня в другую папку на стороне игры\n"
    "(необязательно).\n"
    "- Формат: источник=Цель, через запятую. Пример: plugins=Plugins, config=Config.\n"
    "- Смысл: папка 'plugins' внутри мода связывается с папкой 'Plugins' игры\n"
    "вместо обычной папки Mods.\n"
    "-Оставьте пустым, если ваша игра не разделяет плагины и т.п. от обычных модов."
)
TIP_CP_DEFAULT_MODS_DIR = (
    "Когда включено, моды размещаются прямо в корневой папке профиля, а не\n"
    "в подпапке 'Mods'.\n"
    "Используйте для игр, которые загружают моды прямо из корня игры.\n"
    "Когда включено, поле 'Имя папки модов' выше игнорируется.\n"
    "Оставьте выключенным для типичного случая (моды в отдельной папке Mods)."
)
TIP_CP_SAVE = (
    "Проверить и сохранить этот профиль.\n"
    "Новые профили сразу появляются в выпадающем списке; редактирование обновляет\n"
    "существующую запись.\n"
    "Сохраняется в custom_profiles.json в вашей корневой папке CZT."
)
TIP_CP_DELETE = (
    "Удалить выбранный пользовательский профиль.\n"
    "Это удаляет только запись профиля --> НЕ удаляет установленные моды\n"
    "или файлы на диске.\n"
    "Доступно только для созданных вами пользовательских профилей (не встроенных)."
)
TIP_CP_CLOSE = (
    "Закрыть это окно.\n"
    "Несохраненные изменения в форме отменяются; уже сохраненные профили сохраняются."
)

# Custom Profiles - status / dialog messages
MSG_ENGINE_EXPERIMENTAL = (
    "⚠ Экспериментальный движок: не протестирован на реальной игре. Ожидайте только базовое "
    "развертывание файлов (без специальной маршрутизации или управления порядком загрузки)."
)
MSG_NO_ROOT_FOLDER = "Сначала задайте корневую папку CZT (Первоначальная настройка)."
MSG_APPID_NUMERIC = "Steam App ID должен быть числовым."
DLG_TITLE_DELETE_PROFILE = "Удалить профиль"
MSG_CONFIRM_DELETE_PROFILE = (
    "Удалить пользовательский профиль '{name}'?\nЭто не удаляет установленные моды."
)

# Custom Profiles - validation / persistence messages
MSG_NAME_EMPTY = "Имя профиля не может быть пустым."
MSG_NAME_INVALID_CHARS = 'Имя не может содержать: < > : " / \\ | ? *'
MSG_NAME_RESERVED = "'{name}' - зарезервированное системное имя."
MSG_NAME_TRAILING = "Имя не может заканчиваться пробелом или точкой."
MSG_NAME_BUILTIN = "'{name}' - имя встроенного профиля; выберите другое."
MSG_NAME_EXISTS = "Пользовательский профиль с именем '{name}' уже существует."
MSG_SELECT_ENGINE = "Выберите движок для этого профиля."
MSG_UNKNOWN_ENGINE = "Неизвестный движок: '{engine}'."
MSG_SAVE_FAILED = "Не удалось записать custom_profiles.json (проверьте права доступа к папке)."
MSG_PROFILE_SAVED = "Профиль '{name}' сохранен."
MSG_NOT_CUSTOM = "'{name}' не является пользовательским профилем."
MSG_DELETE_UPDATE_FAILED = "Не удалось обновить custom_profiles.json."
MSG_PROFILE_DELETED = "Профиль '{name}' удален."

FORZATECH_DESC = (
    "Движок наложения свободных файлов для Forza Horizon. Моды - это папки (не паки) "
    "с деревом 'media', которое переопределяет ванильный контент, аддитивным деревом 'mediapc', "
    "и/или корневыми файлами вроде прокси version.dll. CZT объединяет все включенные "
    "моды в одно наложение в порядке загрузки (верхний выигрывает конфликты), затем связывает его с "
    "игрой как соединение для каждой папки верхнего уровня плюс симлинк для каждого корневого файла. "
    "Где уже существует настоящая ванильная папка, накладывает по файлам и создает резервную копию "
    "того, что перекрывает, как '.czt_orig'."
)

TECHLAND_DESC = (
    "Слотовый .pak-движок для C-Engine от Techland (Dying Light 1/2, The "
    "Beast). Порядок загрузки берется из нумерованных имен data##.pak, с низкими "
    "слотами, зарезервированными для игры, и верхним диапазоном, открытым для модов. CZT связывает "
    "каждый .pak мода в плоскую папку ресурсов игры и переименовывает его в "
    "свободный слот. Нативные плагины .dll/.asi связываются рядом с exe игры. Без "
    "папки Content и отдельного аудио - аудио идет внутри паков."
)

UNREAL_DESC = (
    "Pak-движок для Unreal Engine 4/5. Обрабатывает устаревшие .pak плюс контейнеры IoStore "
    ".utoc/.ucas (и файлы .sig для игр с подписанными паками). CZT связывает "
    "паки в папку модов игры (LogicMods, ~mods или Mods) и переименовывает "
    "их по номеру pakchunk для задания порядка загрузки. Поддерживает модлоадеры .dll/.asi "
    "вроде UE4SS, связанные с exe, плюс опциональное аудио Wwise (.bnk/.wem) и FMOD "
    "(.bank)."
)

UNITY_DESC = (
    "Движок на основе модлоадеров для Unity (BepInEx, MelonLoader, UnityModManager) "
    "- загрузчики по-прежнему управляют порядком загрузки, а не движок. Моды - обычно .dll-плагины или "
    "пакеты ассетов .bundle/.unity3d. CZT связывает .dll-моды в папку "
    "модов загрузчика и направляет известные папки верхнего уровня (напр. 'plugins') в их "
    "эквиваленты в корне игры. Опциональное аудио FMOD/Wwise; без концепции паков или "
    "папки Content."
)
