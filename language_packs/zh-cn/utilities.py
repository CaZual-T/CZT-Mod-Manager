...
LOG_HOTKEY_LINK_DONE = "[F7-LINK] {game_name}：模组已成功链接到 {path}（Hotkey - 不启动）"
LOG_HOTKEY_UNLINK_DONE = "[F8-UNLINK] {game_name}：所有模组已成功取消链接。"
...
LOG_LOADER_RENAMED = "[Loader] 已将 '{mod}' 中的 {name} 重命名为 {rel} 以进行代理加载。"
...
LOG_LOADER_NOT_FOUND = "[Loader] 未找到内置加载器：{path}；已跳过。"
LOG_LOADER_INJECTED = "[Loader] 已安装 CZTMM '{name}' 代理加载器。"
LOG_LOADER_INJECT_FAILED = "[Loader] 注入内置加载器失败：{error}"
...
LOG_FORZATECH_AUTOSCOPE_CARBIN_NAME = "[ForzaTech][autoscope] 在 '{archive}' 中检测到 .carbin '{carbin}' -> 将车辆包命名为 '{name}'。"
...
LOG_FORZATECH_DEPLOYED = "[ForzaTech] 已将 {count} 个 overlay 对象部署到 {path}。"
...
LOG_FORZATECH_REMOVED = "[ForzaTech] 已从 {path} 移除 {count} 个 overlay 对象。"
