...
- User Setup tab (TabMainMenu):
  - Root layout: `plugin_api.user_setup_tab.layout()` (created in [gui_/gui_Tab_User_Setup_ui.py](gui_/gui_Tab_User_Setup_ui.py)).
  - Pathing menu layout: `plugin_api.user_setup_tab.path_mode_layout` (explicitly exposed for plugins in [tabs/MainMenu/_Main_MENU.py](tabs/MainMenu/_Main_MENU.py#L52-L104)).
  - Button row layout: `plugin_api.user_setup_tab.ui.gridLayout_2` (the bottom row containing `btnUpdateCZT`, `btnManageMods`, etc.).
- Manage Mods tab:
  - Mods grid: `plugin_api.mod_manager_tab.mods_grid` (set from `ui.mods_grid`) in [tabs/ModManager/_Main_MNGR.py](tabs/ModManager/_Main_MNGR.py#L320-L335).
  - Progress bar layout: `plugin_api.mod_manager_tab.ui.progress_layout` (from [gui_/gui_Tab_Manage_Mods_ui.py](gui_/gui_Tab_Manage_Mods_ui.py)).
- Settings tab:
...
[!NOTE]: You can use dev_ui_scanner_plugin to list every ui element variable.
